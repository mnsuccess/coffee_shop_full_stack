...
AUTH0_DOMAIN = 'manasse.us.auth0.com'
ALGORITHMS = ['RS256']
...
def get_token_auth_header():
    ...
    if not authentication_header:
        raise AuthError({
            ...
            , 401)
    headers = authentication_header.split(' ')
...
def verify_decode_jwt(token):
    ...
    jw_ks = json.loads(json_url.read())
    ...
    if rsa_key:
        try:
            ...
            raise AuthError({
                'code': 'invalid_header',
                'description': 'can not parse authentication token.'
            }, 400)
    raise AuthError({
        'code': 'invalid_header',
